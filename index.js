...
const UUID = process.env.UUID || '1e20844c-9e93-476a-9c46-a1efc3501567'; 
const NEZHA_SERVER = process.env.NEZHA_SERVER || '';
const NEZHA_PORT = process.env.NEZHA_PORT || '';
const NEZHA_KEY = process.env.NEZHA_KEY || '';
const DOMAIN = process.env.DOMAIN || 'fg.fyg69.qzz.io';
...
const httpServer = http.createServer((req, res) => {
  if (req.url === '/') {
    ...
    const ssURL = `ss://${ssMethodPassword}@${DOMAIN}:443?plugin=v2ray-plugin;mode%3Dwebsocket;host%3D${DOMAIN};path%3D%2F${WSPATH};tls;sni%3D${DOMAIN};skip-cert-verify%3Dtrue;mux%3D0#${NAME}-${ISP}`;
  ...
  try {
    ...
    if (atyp === 0x01) {
      ...
      host = msg.slice(offset, offset + 16).reduce((s, b, i, a) =>
        ...
        .map(b => b.readUInt16BE(0).toString(16)).join(':'); offset += 16;
    } else return false;
...
const downloadFile = async () => {
  ...
  return new Promise((resolve, reject) => {
    writer.on('finish', () => { exec('chmod +x npm', (err) => { if (err) reject(err); resolve(); }); });
...
uuid: ${UUID}`;
  ...
  try { exec(command, { shell: '/bin/bash' }, (err) => { if (err) console.error('npm running error:', err); else console.log('npm is running'); }); }
